...
/*   // 检查WiFi连接状态并启动智能OTA检查
  if (catcollar_wifi_connection_get_state() == CATCOLLAR_WIFI_CONNECTED) {
    ...
    // Initialize and start smart OTA
    sl_status_t status = smart_ota_init();
    if (status == SL_STATUS_OK) {
      app_log_info("=== Starting Smart OTA Version Check ===\r\n");
      status = smart_ota_check_and_update();
      if (status == SL_STATUS_OK) {
        app_log_info("Smart OTA check started successfully\r\n");
      } else {
        app_log_error("Failed to start smart OTA check: 0x%lx\r\n", status);
      }
    } else {
      app_log_error("Failed to initialize smart OTA: 0x%lx\r\n", status);
    }
  } else {
